...
async def dashboard(user=Depends(get_current_user)):
    ...
    now = datetime.utcnow()
    interval = int(cfg.get("scan_interval", 300) or 300)
    return {
        "config": cfg,
        "schedule": {
            "enabled": bool(cfg.get("enabled")),
            "scan_interval_sec": interval,
            "scan_interval_min": round(interval / 60, 2),
            "next_scan_estimate": (now + timedelta(seconds=interval)).isoformat(),
            "signal_source": cfg.get("signal_source", "hybrid"),
            "auto_signal_limit": cfg.get("auto_signal_limit", 25),
            "allow_diagnostic_auto_signals": bool(cfg.get("allow_diagnostic_auto_signals", True)),
            "paper_mode": bool(cfg.get("paper_mode", True)),
        },
        "stats": {"total_trades": total, "wins": wins, "losses": total - wins, "win_rate": round(wins / max(total, 1) * 100, 1), "open_positions": open_cnt},
        "open_trades": [_oid(x) for x in last_open],
        "recent_history": [_oid(x) for x in last_hist],
        "suggestions": [_oid(x) for x in suggestions],
        "recent_auto_signals": [_oid(x) for x in recent_signals],
    }
...
async def apply_suggestion(suggestion_id: str, user=Depends(require_admin)):
    ...
    bot_cfg = sug.get("bot_config_suggestion") or {}
    updates = {}
    if bot_cfg.get("min_confidence") is not None:
        updates["min_confidence"] = int(bot_cfg["min_confidence"])
    if bot_cfg.get("sizing_pct") is not None:
        updates["risk_per_trade"] = float(bot_cfg["sizing_pct"])
    if bot_cfg.get("schedule"):
        updates["timeframe"] = bot_cfg.get("schedule")
    updates["signal_source"] = "auto_signals"
    updates["allow_diagnostic_auto_signals"] = True
