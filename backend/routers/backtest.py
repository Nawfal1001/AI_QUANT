"""Backtest router — async jobs, multi-timeframe tests, and Optuna optimization."""
import asyncio, uuid
...
async def _run_one(job_id,req,user,ticker,asset_type,capital,days,interval,index=1,total=1):
    strategy=req.get("strategy","ensemble"); end=datetime.now().strftime("%Y-%m-%d"); start=(datetime.now()-timedelta(days=days)).strftime("%Y-%m-%d")
    ...
    for i,t in enumerate(trades[:100],1): await _job_log(job_id,f"[{interval}] Trade {i}: {t.get('side')} entry {t.get('entry_date')} @ {t.get('entry_price')} → exit {t.get('exit_date')} @ {t.get('exit_price')} | PnL ${t.get('pnl')} ({t.get('pnl_pct')}%) | {t.get('exit_reason')}",data={**t,"interval":interval})
...
async def _run_backtest_job(job_id,req,user):
    try:
        ticker,asset_type,capital,days,intervals=_clean_req(req); await _set_progress(job_id,"running",5)
        if req.get("optimize") or req.get("use_optuna"):
            await _job_log(job_id,f"Starting Optuna optimization + final backtest for {ticker}. Trials: {int(req.get('optuna_trials', req.get('trials',25)))}")
            ...
            opt=await optimize_backtest({**req,"intervals":intervals},user,log_fn=lf,progress_fn=pf)
...
async def run(req:dict,user=Depends(get_current_user)):
    ticker,asset_type,capital,days,intervals=_clean_req(req); interval=intervals[0]; end=datetime.now().strftime("%Y-%m-%d"); start=(datetime.now()-timedelta(days=days)).strftime("%Y-%m-%d")
    return await run_backtest(ticker=ticker,asset_type=asset_type,start_date=start,end_date=end,interval=interval,initial_capital=capital,risk_per_trade=float(req.get("risk_per_trade",0.02)),min_confidence=int(req.get("min_confidence",55)),sl_atr_mult=float(req.get("sl_atr_mult",2.0)),tp_atr_mult=float(req.get("tp_atr_mult",3.0)),fee_bps=float(req.get("fee_bps",5)),slippage_bps=float(req.get("slippage_bps",3)),spread_bps=float(req.get("spread_bps",2)),max_hold_bars=int(req.get("max_hold_bars",30)),strategy=req.get("strategy","ensemble"))
@router.post("/compare")
async def compare(req:dict,user=Depends(get_current_user)):
    ...
    return await run_compare(ticker=ticker,asset_type=asset_type,start_date=start,end_date=end,interval=intervals[0],initial_capital=capital,strategies=raw,risk_per_trade=float(req.get("risk_per_trade",0.02)),min_confidence=int(req.get("min_confidence",55)))
@router.get("/history")
