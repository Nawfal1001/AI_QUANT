...
async def health(user=Depends(get_current_user)):
    ...
    checks = [
        ...
        item('Gemini AI', present('GEMINI_API_KEY','GEMINY_API_KEY','GOOGLE_API_KEY'), detail=os.getenv('GEMINI_MODEL','gemini-1.5-flash')),
    ...
    return {'status': 'ok' if all(c['ok'] or c['level']=='warning' for c in checks) else 'degraded', 'live_trading': os.getenv('LIVE_TRADING_ENABLED','false').lower() in {'1','true','yes'}, 'auto_signal_scanner': os.getenv('AUTO_SIGNAL_SCANNER_ENABLED','true'), 'checks': checks}
...
async def ai_test(req: dict, user=Depends(get_current_user)):
    ...
    try:
        ...
        return {'ok': True, 'latency_ms': int((time.time()-started)*1000), 'response': text}
    except Exception as e:
        return {'ok': False, 'latency_ms': int((time.time()-started)*1000), 'error': str(e)}
