...
async def _find_trade(source: str, trade_id: str, user: Dict[str, Any]):
    ...
    candidates = []
    try:
        candidates.append({**q, "_id": ObjectId(trade_id)})
    except Exception:
        pass
...
async def _fetch_candles(trade: Dict[str, Any], source: str):
    ...
    if not ticker:
        return []
    atype = _asset_type(trade)
    interval = _interval_for_trade(trade)
    opened = _parse_dt(_field(trade, "opened_at", "entry_time", "created_at", "timestamp")) or datetime.utcnow() - timedelta(days=5)
    closed = _parse_dt(_field(trade, "closed_at", "exit_time", "updated_at")) or datetime.utcnow()
    start = opened - timedelta(days=3)
    end = closed + timedelta(days=2)
    try:
...
def _chart_payload(source: str, coll: str, trade: Dict[str, Any], candles):
    entry = _num(_field(trade, "entry_price", "entry", "open_price", "price"))
    exitp = _num(_field(trade, "close_price", "exit_price", "closed_price"))
    sl = _num(_field(trade, "sl", "stop_loss", "stop"))
    tp = _num(_field(trade, "tp", "take_profit", "target"))
    opened = _parse_dt(_field(trade, "opened_at", "entry_time", "created_at", "timestamp"))
    closed = _parse_dt(_field(trade, "closed_at", "exit_time"))
    signal = str(_field(trade, "signal", "side", default="") or "").upper()
    is_sell = "SELL" in signal or signal == "SHORT"
    markers = []
    if entry is not None:
        markers.append({"time": _marker_time(opened, candles), "position": "aboveBar" if is_sell else "belowBar", "color": "#58a6ff", "shape": "arrowDown" if is_sell else "arrowUp", "text": f"ENTRY {entry:.4f}"})
    if exitp is not None:
        markers.append({"time": _marker_time(closed, candles), "position": "belowBar" if is_sell else "aboveBar", "color": "#e3b341", "shape": "circle", "text": f"EXIT {exitp:.4f}"})
    lines = []
    for key, price, color, title in [("entry", entry, "#58a6ff", "Entry"), ("tp", tp, "#3fb950", "TP"), ("sl", sl, "#f85149", "SL"), ("exit", exitp, "#e3b341", "Exit")]:
        if price is not None:
            lines.append({"key": key, "price": price, "color": color, "title": title})
