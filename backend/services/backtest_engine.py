    ...
    try:
        ...
        for dt, vals in series.items():
            ...
            if ts < start_ts or ts > end_ts:
                continue
            close_val = vals.get("4. close") or vals.get("5. adjusted close")
            volume_val = vals.get("6. volume") or vals.get("5. volume") or vals.get("volume") or 0
            rows.append({"date": ts, "open": vals.get("1. open"), "high": vals.get("2. high"), "low": vals.get("3. low"), "close": close_val, "volume": volume_val})
        if not rows:
            log.warning(f"Alpha Vantage history for {ticker} had 0 rows inside {start}..{end}")
            return None
...
def _fetch_stock_history(ticker, start, end, interval="1d"):
    ...
    try:
        ...
        if df is not None and len(df) > 0:
            return df
        log.warning(f"Yahoo download returned 0 stock candles for {symbol}; trying Ticker.history")
        hist = yf.Ticker(symbol).history(start=start, end=end, interval=interval, auto_adjust=False, raise_errors=False)
        df = _normalize_history_frame(hist)
        if df is not None and len(df) > 0:
            return df
        log.warning(f"Yahoo/yfinance returned no stock history for {symbol}; trying Alpha Vantage fallback")
    except Exception as e:
        log.warning(f"Yahoo/yfinance fetch failed for {symbol}; trying Alpha Vantage fallback: {e}")
    return _fetch_alpha_vantage_stock_history(symbol, start, end, interval)
...
def _fetch_crypto_history(symbol, start, end, interval="1d"):
    try:
        ...
        base = str(symbol).upper().replace("/USDT", "").replace("USDT", "")
        pair = f"{base}/USDT"
