...
def _fetch_alpha_vantage_stock_history(ticker,start,end,interval="1d"):
    ...
    try:
        ...
        if is_daily: params={"function":"TIME_SERIES_DAILY_ADJUSTED","symbol":ticker,"outputsize":"full","apikey":key}; time_key="Time Series (Daily)"
        else:
            av_interval=interval if interval in {"1min","5min","15min","30min","60min"} else "15min"; params={"function":"TIME_SERIES_INTRADAY","symbol":ticker,"interval":av_interval,"outputsize":"full","apikey":key}; time_key=f"Time Series ({av_interval})"
        r=httpx.get("https://www.alphavantage.co/query",params=params,timeout=20); r.raise_for_status(); data=r.json(); series=data.get(time_key) or {}
        if not series: log.warning(f"Alpha Vantage returned no history for {ticker}: {data.get('Note') or data.get('Information') or data.get('Error Message') or 'empty response'}"); return None
        start_ts=pd.Timestamp(start).tz_localize(None); end_ts=pd.Timestamp(end).tz_localize(None)+pd.Timedelta(days=1); rows=[]
...
async def run_backtest(ticker: str, asset_type: str = "stock", start_date: str = None, end_date: str = None, interval: str = "1d", initial_capital: float = 10000, risk_per_trade: float = 0.02, min_confidence: int = 55, sl_atr_mult: float = 2.0, tp_atr_mult: float = 3.0, fee_bps: float = DEFAULT_FEE_BPS, slippage_bps: float = DEFAULT_SLIPPAGE_BPS, spread_bps: float = DEFAULT_SPREAD_BPS, max_hold_bars: int = 30, strategy: str = "ensemble", custom_strategy_def: dict = None):
    if custom_strategy_def is not None:
        ...
        signal_fn=STRATEGIES[strategy]; strategy_name=strategy
    ...
    capital=initial_capital; equity_curve=[{"date":str(df["date"].iloc[0])[:10],"equity":capital}]; drawdown_curve=[{"date":str(df["date"].iloc[0])[:10],"dd_pct":0}]; trades=[]; open_position=None; bars_since_entry=0; peak_equity=capital
    for i in range(50,len(df)):
        row=df.iloc[i]; sig_window=df.iloc[max(0,i-50):i]; sig=signal_fn(sig_window); price=float(row["close"]); high=float(row["high"]); low=float(row["low"]); atr=price*(sig.get("atr_pct",1)/100)
        if open_position:
            ...
            if exit_reason:
                ...
                trades.append({"entry_date":open_position["entry_date"],"exit_date":str(row["date"])[:10],"side":open_position["side"],"entry_price":round(open_position["entry_price_net"],4),"exit_price":round(exec_exit,4),"qty":round(open_position["qty"],4),"pnl":round(pnl,2),"pnl_pct":round(pnl/(open_position["entry_price_net"]*open_position["qty"])*100,2),"exit_reason":exit_reason,"bars_held":bars_since_entry,"confidence":open_position.get("confidence")}); open_position=None; bars_since_entry=0
        if not open_position and sig["confidence"]>=min_confidence and "HOLD" not in sig["signal"]:
            side="BUY" if "BUY" in sig["signal"] else "SELL"; risk_dollars=capital*risk_per_trade; stop_distance=max(atr*sl_atr_mult,price*0.005); qty=risk_dollars/stop_distance; entry_price_net=_apply_costs(price,side,fee_bps,slippage_bps,spread_bps); sl=price-stop_distance if side=="BUY" else price+stop_distance; tp=price+atr*tp_atr_mult if side=="BUY" else price-atr*tp_atr_mult; open_position={"side":side,"entry_price":price,"entry_price_net":entry_price_net,"qty":qty,"sl":sl,"tp":tp,"entry_date":str(row["date"])[:10],"confidence":sig["confidence"]}; bars_since_entry=0
        equity=capital
