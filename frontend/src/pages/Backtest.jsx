...
export default function Backtest() {
  ...
  function payload() {
    const p = { ticker, asset_type: assetType, capital: Number(capital), days: Number(days), interval, strategy, min_confidence: Number(minConf), risk_per_trade: Number(risk) / 100, sl_atr_mult: Number(slMult), tp_atr_mult: Number(tpMult), fee_bps: Number(feeBps), slippage_bps: Number(slipBps), spread_bps: Number(spreadBps) }
  ...
  async function pollJob(id) {
    try {
      ...
      setError(e?.response?.data?.detail || e?.message || 'Could not load backtest job')
    }
  }
  ...
  return <div>
    <PageHeader title="📊 Backtesting" subtitle="Historical simulation with live progress, provider logs, and trade-by-trade output" />
    <Card style={{ marginBottom: 14 }}>
      ...
      <Grid cols={4} minCol={130} gap={12}>
        ...
        <Select label="Interval" value={interval} onChange={e => setInterval_(e.target.value)}><option value="1d">Daily</option><option value="4h">4 Hour</option><option value="1h">1 Hour</option><option value="15m">15 Min</option></Select>
      ...
      <div style={{ marginTop: 14 }}><Button onClick={runBacktest} disabled={running} loading={running} leftIcon={<Play size={14} />}>{running ? 'Running…' : compareMode ? 'Compare Strategies' : 'Run Backtest'}</Button></div>
    </Card>

    {(jobId || running || jobLogs.length > 0) && !compareMode && <Card style={{ marginBottom: 14 }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 8 }}><SectionTitle icon={<Activity size={14} />}>Live Backtest Progress</SectionTitle><div style={{ fontSize: 12, color: tokens.textMuted }}>{jobStatus || 'idle'} · {Math.round(jobProgress)}%</div></div>
      <div style={{ background: tokens.bgInput, borderRadius: 8, height: 9, overflow: 'hidden', marginBottom: 12 }}><div style={{ height: '100%', width: `${Math.min(100, jobProgress)}%`, background: tokens.primary }} /></div>
      <div style={{ maxHeight: 280, overflowY: 'auto', background: '#05080d', border: `1px solid ${tokens.border}`, borderRadius: 10, padding: 10, fontFamily: 'ui-monospace, SFMono-Regular, Menlo, monospace', fontSize: 12 }}>
        {jobLogs.length === 0 && <div style={{ color: tokens.textMuted }}>Waiting for logs...</div>}
        {jobLogs.map((l, i) => <div key={i} style={{ color: l.level === 'error' ? tokens.danger : l.level === 'warning' ? tokens.warning : tokens.textMuted, marginBottom: 5 }}><span style={{ color: '#64748b' }}>{l.ts?.slice(11, 19)}</span> {l.message}</div>)}
      </div>
    </Card>}
    ...
    {result && !error && <>
      <Grid cols={4} minCol={150} gap={10} style={{ marginBottom: 14 }}><Metric label="Total Return" value={`${result.total_return_pct >= 0 ? '+' : ''}${result.total_return_pct}%`} color={signColor(result.total_return_pct)} sub={`$${result.capital_start.toLocaleString()} → $${result.capital_end.toLocaleString()}`} /><Metric label="CAGR" value={`${result.cagr_pct}%`} color={signColor(result.cagr_pct)} /><Metric label="Sharpe" value={result.sharpe} color={result.sharpe >= 1 ? tokens.success : result.sharpe >= 0 ? tokens.warning : tokens.danger} sub={`Sortino ${result.sortino}`} /><Metric label="Max Drawdown" value={`${result.max_drawdown}%`} color={tokens.danger} /><Metric label="Win Rate" value={`${result.win_rate}%`} color={result.win_rate >= 50 ? tokens.success : tokens.warning} sub={`${result.wins}W / ${result.losses}L`} /><Metric label="Profit Factor" value={result.profit_factor} color={result.profit_factor >= 1.5 ? tokens.success : result.profit_factor >= 1 ? tokens.warning : tokens.danger} /><Metric label="Expectancy" value={`$${result.expectancy}`} color={signColor(result.expectancy)} sub="per trade" /><Metric label="Total Trades" value={result.total_trades} sub={`${result.bars} bars`} /></Grid>
      <Card style={{ marginBottom: 14 }}><SectionTitle>📈 Equity Curve</SectionTitle><LineChart data={result.equity_curve} xKey="date" yKey="equity" height={220} /></Card>
      <Card style={{ marginBottom: 14 }}><SectionTitle>📉 Drawdown</SectionTitle><DrawdownChart data={result.drawdown_curve} xKey="date" yKey="dd_pct" height={160} /></Card>
      <Card><div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 10 }}><SectionTitle>📋 Trades ({result.trades?.length || 0})</SectionTitle><Button size="sm" variant="secondary" onClick={() => setShowTrades(!showTrades)}>{showTrades ? 'Hide' : 'Show'}</Button></div>{showTrades && <div style={{ overflowX: 'auto' }}><table style={{ width: '100%', fontSize: 11, color: tokens.textMuted }}><thead><tr style={{ borderBottom: `1px solid ${tokens.border}`, textAlign: 'left' }}>{['#', 'Entry', 'Exit', 'Side', 'Entry $', 'Exit $', 'PnL', 'PnL %', 'Reason', 'Bars'].map(h => <th key={h} style={{ padding: '6px 8px', fontWeight: 500 }}>{h}</th>)}</tr></thead><tbody>{result.trades?.map((t, i) => <tr key={i} style={{ borderBottom: `1px solid ${tokens.border}` }}><td style={{ padding: '6px 8px' }}>{i + 1}</td><td style={{ padding: '6px 8px' }}>{t.entry_date}</td><td style={{ padding: '6px 8px' }}>{t.exit_date}</td><td style={{ padding: '6px 8px', color: t.side === 'BUY' ? tokens.success : tokens.danger, fontWeight: 600 }}>{t.side}</td><td style={{ padding: '6px 8px' }}>${t.entry_price}</td><td style={{ padding: '6px 8px' }}>${t.exit_price}</td><td style={{ padding: '6px 8px', color: signColor(t.pnl), fontWeight: 600 }}>${t.pnl}</td><td style={{ padding: '6px 8px', color: signColor(t.pnl_pct) }}>{t.pnl_pct > 0 ? '+' : ''}{t.pnl_pct}%</td><td style={{ padding: '6px 8px' }}><Badge color={t.exit_reason === 'TP' ? tokens.success : t.exit_reason === 'SL' ? tokens.danger : tokens.textMuted}>{t.exit_reason}</Badge></td><td style={{ padding: '6px 8px' }}>{t.bars_held}</td></tr>)}</tbody></table></div>}</Card>
    </>}
