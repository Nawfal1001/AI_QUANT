    ...
    <div>
      ...
      <div style={{ ...card, marginBottom: 16 }}>
        <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap', alignItems: 'flex-end' }}>
          <div><div style={{ fontSize: 11, color: '#8b949e', marginBottom: 4 }}>TICKER</div><input value={ticker} onChange={e => setTicker(e.target.value.toUpperCase())} onKeyDown={e => e.key === 'Enter' && run()} style={{ background: '#0d1117', border: '1px solid #21262d', borderRadius: 7, padding: '8px 12px', color: '#e2e8f0', fontSize: 14, width: 110, outline: 'none' }} placeholder="AAPL" /></div>
          ...
          <div><div style={{ fontSize: 11, color: '#8b949e', marginBottom: 4 }}>TIMEFRAME</div><select value={tf} onChange={e => setTf(e.target.value)} style={{ background: '#0d1117', border: '1px solid #21262d', borderRadius: 7, padding: '8px 10px', color: '#e2e8f0', fontSize: 13, cursor: 'pointer', outline: 'none' }}>{['scalping', 'intraday', 'swing', 'position'].map(t => <option key={t} value={t}>{t}</option>)}</select></div>
          <div><div style={{ fontSize: 11, color: '#8b949e', marginBottom: 4 }}>MODE</div><select value={mode} onChange={e => setMode(e.target.value)} style={{ background: '#0d1117', border: '1px solid #21262d', borderRadius: 7, padding: '8px 10px', color: '#e2e8f0', fontSize: 13, cursor: 'pointer', outline: 'none' }}><option value="single">Single TF</option><option value="multi">All Timeframes</option></select></div>
          <button onClick={run} disabled={loading} style={{ padding: '9px 20px', borderRadius: 7, border: 'none', background: loading ? '#21262d' : '#1f6feb', color: loading ? '#8b949e' : '#fff', fontWeight: 600, fontSize: 13, cursor: loading ? 'not-allowed' : 'pointer', display: 'flex', alignItems: 'center', gap: 6 }}><Search size={14} /> {loading ? 'Scanning...' : 'Scan Signal'}</button>
        </div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 14 }}>
          <div style={card}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 14 }}>
              <div><div style={{ fontSize: 22, fontWeight: 700, color: '#e2e8f0' }}>{s.ticker || ticker.toUpperCase()}</div><div style={{ fontSize: 12, color: '#8b949e' }}>{fmtMoney(s.price)} · {s.timeframe || tf} · {s.regime || '—'}</div></div>
              <div style={{ textAlign: 'right' }}><div style={{ fontSize: 16, fontWeight: 700, color: sig_color(s.signal), background: sig_bg(s.signal), padding: '4px 12px', borderRadius: 10 }}>{s.signal || 'HOLD'}</div><div style={{ fontSize: 12, color: '#8b949e', marginTop: 4 }}>Confidence: {fmtPct(s.confidence)}</div></div>
            </div>
            <ConfBar value={s.confidence} />
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 8, marginTop: 14 }}>
              {[['Price', fmtMoney(s.price)], ['SL', fmtMoney(s.sl)], ['TP', fmtMoney(s.tp)], ['ATR', fmtMoney(s.atr)], ['Regime', s.regime || '—'], ['Bayesian', `${Math.round(Number(s.bayesian?.p_buy ?? 0.5) * 100)}% BUY`]].map(([k, v]) => <div key={k} style={{ background: '#0d1117', borderRadius: 7, padding: '8px 10px' }}><div style={{ fontSize: 10, color: '#8b949e' }}>{k}</div><div style={{ fontSize: 12, color: '#e2e8f0', fontWeight: 500, marginTop: 2 }}>{v}</div></div>)}
            </div>
          </div>
          <div style={card}>
            <div style={{ fontSize: 13, fontWeight: 600, color: '#e2e8f0', marginBottom: 12 }}>📊 Indicator Breakdown</div>
            <div style={{ display: 'flex', flexDirection: 'column', gap: 5, maxHeight: 320, overflowY: 'auto' }}>
              {(s.indicators || []).length === 0 && <div style={{ color: '#8b949e', fontSize: 12 }}>No indicator data available for this symbol/timeframe.</div>}
